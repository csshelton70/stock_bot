...
class CryptoRepository(BaseRepository[Crypto]):
    ...
    def get_monitored_symbols(self) -> List[str]:
        ...
        with self.get_session() as session:
            results = (
                session.query(Crypto.symbol)
                .filter(Crypto.monitored)
                .all()  # type:ignore
            )
    ...
    def get_prices_bulk(self, symbols: List[str]) -> Dict[str, float]:
        ...
        with self.get_session() as session:
            cryptos = (
                session.query(Crypto).filter(Crypto.symbol.in_(symbols)).all()
            )  # type:ignore
    ...
    def upsert_crypto_data(self, crypto_data: List[Dict]) -> int:
        ...
        with self.get_session() as session:
            for data in crypto_data:
                ...
                existing = (
                    session.query(Crypto).filter_by(symbol=symbol).first()
                )  # type:ignore
